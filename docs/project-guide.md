...
| 中文易错字检测器 | `/tools/typo-checker` | 内置500+组常见易错词对照库（正确↔错误），粘贴文本自动扫描匹配，高亮标注错别字，显示正确写法，支持一键全部替换，统计错误数量 |
| 数据脱敏工具 | `/tools/data-masking` | 内置PII正则规则库：手机号/身份证/银行卡/邮箱/IP地址/姓名/车牌号/护照号等，多种脱敏策略：全部掩码/部分掩码/首尾保留/哈希替换，实时预览脱敏前后对比，支持自定义正则规则 |
| 音频基频检测器 | `/tools/pitch-detector` | Web Audio API + 自相关算法(ACF)检测麦克风实时音频的基频，显示Hz值 + 最近音名(C4/D4/E4...) + 偏差(音分)，可视化音高仪表盘/指针，用于乐器调音或唱歌练习，波形实时显示，检测历史记录 |
| 文本转语音朗读器 | `/tools/text-to-speech` | Web Speech API speechSynthesis，文本输入区 + 语言选择 + 语音角色选择，滑块调节语速/音调/音量，播放/暂停/恢复/停止按钮，进度条显示，快捷键支持（空格播放/暂停，S键停止） |
| 图像隐写工具 | `/tools/image-steganography` | 使用 LSB 隐写术在图片像素中隐藏秘密文本，也可从图片中提取隐藏信息。支持拖拽上传图片、输入/输出隐藏文本、导出含密 PNG。纯前端实现，数据不离开设备 |
| SVG 无缝图案生成器 | `/tools/svg-pattern-generator` | 创建可平铺的 SVG 无缝图案，自定义形状类型（矩形/圆形/三角形/菱形/十字/六边形/波浪线/星形）、颜色、大小、旋转角度、密度参数，实时预览平铺效果，导出 SVG 代码和 CSS background-image 代码 |
| 多轨音频混合器 | `/tools/audio-mixer` | 上传多个音频文件，独立调节每轨的音量、声像（左右平衡），支持静音/独奏、拖拽排序、每轨波形显示、播放/暂停/停止控制，使用 OfflineAudioContext 离线渲染导出 WAV 文件 |
